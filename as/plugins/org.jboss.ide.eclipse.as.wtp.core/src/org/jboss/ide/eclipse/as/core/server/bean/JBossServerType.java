		...
		public String getFullVersion(File location, File systemFile) {
			...
			if (fullVersion != null && fullVersion.length() >= 5) {
				...
				if (pattern.matcher(check).matches()) {
					...
					String version = ServerBeanLoader.getFullServerVersionFromZip(new File(location, runJar));
					if (version != null) {
						return version;
					}
				}
			...
			if (fullVersion != null && fullVersion.length() >= 5) {
				...
				if (pattern.matcher(check).matches()) {
					String runJar = JBossServerType.JBOSS_AS_PATH + File.separatorChar + 
						JBossServerType.BIN_PATH+ File.separatorChar + JBossServerType.RUN_JAR_NAME;
					String version = null;
					version = ServerBeanLoader.getFullServerVersionFromZip(new File(location, runJar));
					if (version == null) {
						runJar = JBossServerType.JBOSS_AS_WEB_PATH + File.separatorChar + 
								JBossServerType.BIN_PATH+ File.separatorChar + JBossServerType.RUN_JAR_NAME;
						version = ServerBeanLoader.getFullServerVersionFromZip(new File(location, runJar));
					}
					if (version != null) {
